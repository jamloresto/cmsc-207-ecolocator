    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          <button
            type="button"
            className="absolute inset-0 bg-black/40 h-screen w-screen"
            onClick={() => setOpen(false)}
            aria-label="Close admin menu overlay"
          />

          <div className="bg-card absolute top-0 left-0 flex h-full w-72 flex-col shadow-xl">
            ...
            <nav className="flex-1 space-y-1 p-4 bg-background">
              {items.map((item) => {
                const isActive =
                  pathname === item.href ||
